...
Текущий PR фиксирует только структуру проекта и границы будущих модулей.

Kotlin-файлы в `core:*`, `feature:*` и `ui:*` остаются `Placeholder.kt` без
бизнес-логики, инфраструктурной логики, Decompose component tree, DI-графа,
Git/MCP/storage/network реализаций, UI-flow и временных domain-моделей.

Допустимо оставить только минимальные main/entrypoint-классы target'ов:

- desktop `main`;
- IntelliJ plugin `ToolWindowFactory` с простой заглушкой окна.

Пользовательские сценарии ниже описывают целевое направление продукта, но не
входят в реализацию текущего placeholder-скелета.
...
- общие project convention, Kotlin JVM и Kotlin Multiplatform настройки живут в included build `build-logic`;
- root `build.gradle.kts` не конфигурирует subprojects напрямую;
- модули применяют convention plugins и локально объявляют только свои зависимости и target-specific plugin/configuration.
...
В текущем PR содержит только минимальную desktop-точку входа без запуска UI-flow.
...
В текущем PR содержит только `ToolWindowFactory` с простой заглушкой окна без plugin-flow.
...
В текущем PR содержит только placeholder-файл; Decompose component tree добавляется отдельной задачей.
...
В текущем PR содержит только placeholder-файл; DI wiring добавляется отдельной задачей.
...
В текущем PR содержит только placeholder-файлы; Git-контракты и реализации добавляются отдельной задачей.

### `core:code-render`

Модели rendered code, diff-фрагменты, mapping Git diff -> UI model, позиционные модели для перехода к файлу и строке.

В текущем PR содержит только placeholder-файл.
...
- Текущий PR не реализует flow-логику, domain-модели, DI, Decompose-навигацию, Git/MCP/storage/network adapters или UI layout.
- Kotlin source-файлы в non-entrypoint модулях называются `Placeholder.kt`, чтобы сохранить видимую структуру будущих зон ответственности.
...
- Kotlin source set'ы содержат `Placeholder.kt` в ожидаемых пакетах.
- В Kotlin-коде нет временной бизнес-логики, инфраструктурной логики, UI-flow логики или domain-моделей.
- Сохранены только минимальный desktop `main` и IntelliJ `ToolWindowFactory`.
- Core/api placeholder-слои не зависят от desktop/plugin UI.
- `app:desktop` собирает минимальный desktop entrypoint.
- `app:intellij-plugin` собирает минимальный `ToolWindowFactory` с заглушкой окна.
- Общие Gradle project/Kotlin настройки вынесены в convention plugins.
...
- UI layout не валидируется визуально в текущем PR, потому что UI-flow не реализуется.
