...
- описывать route/component contracts и переходы между экранами;
- потреблять Git/storage только через явные platform-neutral interfaces;
- не протаскивать MCP и target-specific review-domain данные до появления соответствующих задач;
...
- содержит screen-модели `ProjectSelectionModel` и `LocalReviewModel`;
- содержит start-mode contract `MissionReviewStart`;
- зависит от `core:git` и `core:storage` только как от common API;
- не содержит Compose, Swing, IntelliJ Platform, MCP или JVM filesystem API.
...
- передает desktop directory picker, recent-projects store и JVM Git inspector;
...
- передает JVM Git inspector для текущего IDE project;
...
`ProjectSelectionComponent` используется desktop target'ом для выбора локальной директории проекта и открытия recent projects.

`LocalReviewComponent` представляет первый экран локального ревью: project path, файловое дерево, changed files и diff выбранного файла. Review comments, MCP session и code navigation пока не входят в модель.
...
Экран выбора проекта показывает кнопку `Choose project` и recent projects. Выбор директории выполняется target-specific picker'ом из `app:desktop`, после чего root переходит в `LocalReviewChild` с выбранным project path.
...
Если `project.basePath` пустой, UI показывает сообщение:

```text
Open a project in IntelliJ IDEA before using Local review.
```

Отсутствие Git repository обрабатывается отдельно от отсутствия открытого проекта. Для проекта без Git UI показывает сообщение уровня:
...
- Git/storage реализации не должны появляться в navigation-слое;
- MCP детали не должны появляться в navigation-слое до появления явных contracts.
